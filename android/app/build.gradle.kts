...
// The file must NOT be committed to version control. Create it for local release builds:
...
android {
    ...
    buildTypes {
        release {
            // DO NOT sign with the debug keystore in production. A real keystore
            // must be configured via key.properties (or CI secrets). When no
            // keystore is present, release builds are left unsigned so a faulty
            // debug-signed APK can never be published by accident.
